...
interface Props {
    ...
    pedidos: Pedidos[]; // Pass all pedidos to extract unique providers
    mode?: 'print' | 'export';
}

const PedidosPrintModal: React.FC<Props> = ({ isOpen, onClose, onConfirm, pedidos, mode = 'print' }) => {
    const [selectedProvider, setSelectedProvider] = useState('');
    const [providers, setProviders] = useState<string[]>([]);

    useEffect(() => {
        if (isOpen && pedidos.length > 0) {
            // Extract unique provider names
            const uniqueProviders = Array.from(new Set(
                pedidos
                    .map(p => p.proveedor?.proveedor)
    ...
    const title = isExport ? 'Exportar Lista de Pedidos' : 'Imprimir Lista de Pedidos';
                            ...
                            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div className="mt-2">
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                                        Seleccione el proveedor del cual desea imprimir los pedidos. Si deja la opción por defecto, se imprimirán todos los pedidos visibles.
                                    </p>
...
export default PedidosPrintModal;
